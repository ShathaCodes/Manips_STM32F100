...
public:
     ...
     */
		mycPinAsOutput(PinName pin) :thepin(pin) { 
			mycgpio_init_out_def  (pin); 
		}
...
/***** Create a PinAsOutput connected to the specified pin
     ...
     */
    mycPinAsOutput(PinName pin, PinOutMode mode):thepin(pin) {
        mycgpio_init_out (pin, mode);
    }
...
/***** Create a PinAsOutput connected to the specified pin
     ...
     *  @param mode the initial mode of the pin
     */
    mycPinAsOutput(PinName pin, PinOutMode mode, PinSpeed speed):thepin(pin) {
        mycgpio_init_out_ex (pin, mode, speed);
    }
...
/***** Read the input, represented as 0 or 1 (int)
     ...
     */
    int mycread() {
        return mycgpio_read_out(thepin);
    }	
...
/***** Write to the output pin, represented as 0 or 1
     ...
     *    0 for logical 0, 1 for logical 1
     */		
		    void mycwrite(int value) {
					mycgpio_write(thepin,value);
				}
